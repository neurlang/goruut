...
type IHashtronHomonymSelectorRepository interface {
	Select(isReverse bool, lang string, sentence []map[uint32]string) (ret [][4]uint32)
}
...
func (r *HashtronHomonymSelectorRepository) Select(isReverse bool, lang string, sentence []map[uint32]string) (ret [][4]uint32) {
	var reverse string
	if isReverse {
		reverse = "_reverse"
	}
	...
	for i, mapping := range sentence {
		...
		for k, v := range mapping {
			...
			strkey = append(strkey, [2]string{v, fmt.Sprint(k)})
		}
	...
	for i := range ai_sentence.Sentence {
		...
		for j := 0; !accept && j < sample.Len(); j++ {
			...
			ret = append(ret, [4]uint32{uint32(i),ai_sentence.Sentence[i].Choices[j][0],ai_sentence.Sentence[i].Choices[j][1],pred})
			if pred == 1 {
				accept = true
